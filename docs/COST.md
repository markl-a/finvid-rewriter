...
| s3 Pass A `gpt-5-mini` | 命中 `03a_pass_a.json` 快取（首次約 4,400 in / 2,300 out，$0.006） | $0 |
| s3 Pass B `gpt-5` × 3 | 每段約 1,440 in / 1,650 out（含推理 token） | $0.0518 |
| s4 TTS edge-tts | 381 字元 | $0 |
| s4 AI 開場鏡頭 HF ZeroGPU（LTX-Video） | 3 段 × 5 秒，共 39 秒 | $0 |
| s4 Pexels 素材 | 23 段（這次執行 0 次搜尋：全部命中快取；首次 23 次） | $0 |
| **合計（目前產出）** | | **$0.099** |

帳本只保留每個 stage「最後一次成功執行」的費用；整個開發過程 17 次執行（s3 提示詞改了 8 版）累計 **$0.385**（`manifest.json` 的 `runs[]`，`finvid costs` 會另印這一行）。

估算 vs 實際的落差：第一版 dry-run 估 $0.083，主要低估了 gpt-5 系列的推理 token 會計入輸出。
已把估算常數校準到實測值（`PASS_A_OUTPUT_TOKENS`、`PASS_B_OUTPUT_TOKENS`），現在 dry-run 估 $0.104。

### 開發迭代的總花費（快取的價值）

第 3 步的閘門與提示詞改了 8 版（重複判斷、時間格式、數字格式、AI 鏡頭描述、素材關鍵字），第 4 步的版面改了更多次。每次重跑只付真的變了的那一步，其餘 cache hit：

| 重做的 stage | 次數 | 每次花費 | 說明 |
|---|---|---|---|
| s2 | 1 | $0.047 | 只轉過一次逐字稿 |
| s3 | 8 | $0.045–0.067 | 後期 Pass A 另有自己的快取，只改 Pass B 提示詞時只付 Pass B |
| s4 | 8 | $0 | TTS 免費、AI 鏡頭依 prompt hash 快取（換版面重渲染 6 段全部命中） |
| **整個開發過程 17 次執行** | | **$0.385** | 無快取會是 17 × $0.11 ≈ $1.9 |
...
| 3 支共 110 秒整支用 AI 影片 API 生成 | 110 s × $0.25 = **$27.5** |
| 只做 3 段 5 秒開場鏡頭，用付費 API | MiniMax 768P 3 × $0.27 = $0.81；Kling v1 3 × $0.18 = $0.54 |
| 8 段全生成 AI 影片 | ≈ **$79** |
